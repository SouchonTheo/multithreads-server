...
public class Tuple extends LinkedList<Serializable> implements Serializable {
    ...
    /**
     * Creates a new tuple. Example : new Tuple(4, 5, "foo", true) -> [ 4 5 "foo"
     * true ] new Tuple(4, Integer.class, "foo".getclass(), Boolean.class) -> [ 4
    ...
    /**
     * Returns true if this tuple matches the given template. Matching rules : a
     * tuple matches a template if all their components match two by two. Two
     * components match : - if they are both values and are equals; - if the
     * template component is a class/interface, and the tuple component is an
     * instance/implementation of this class/interface (Class.isInstance); - if the
     * template component is a class/interface, and the tuple component is a
     * subclass/subinterface of this class/interface (Class.isAsssignableFrom); -
     * recursively if both are tuples.
     *
     * Examples: [ 3 5 "foo" ] matches [ 3 5 "foo" ], [ ?Integer 5 "foo" ], [
     * ?Integer ?Integer ?String ] [ 3 ?Integer [ 6 7 ] [ 7 8 ] ] matches [ ?Integer
     * ?Integer [ ?Integer 7 ] ?Tuple ], [3 ?Integer ?Tuple ?Tuple ]
     *
    ...
    /**
     * Returns true if this tuple (seen as a template) contains <code>t</code>. This
     * is the reverse of {@link #matches(Tuple)}.
     */
     ...
     * cloneable. Le plus simple de passer par une sérialisation/desérialisation ce
     * qui marche pour toutes les classes qui implantent serializable.
     */
    ...
    /**
     * Parses a sequence of words to form a tuple. The initial [ must be missing.
     * 
     * @param stk
     * @return
    ...
    /**
     * Returns a Tuple with a value represented by the specified String. Known
     * values: integer (45, -67), boolean (true, false), string ("toto"), classname
     * (?Integer, ?java.awt.Rectangle), recursive tuple. Examples: [ 3 4 ], [
     * ?Integer "toto" true 78 ?Boolean ], [ ?Integer ?Tuple ], [ [ true 78 ] [ 3 4
     * [ 5 6 ] 7 ] ] For these components, the parsable strings are identical to the
     * printed strings. Note: do not expect the parser to be resilient to arbitrary
     * strings.
     *
